...
import { FiArrowLeft, FiPlay, FiBookmark, FiUsers } from "react-icons/fi"; 
import { FaBookmark } from "react-icons/fa";
import { useStats } from "../context/StatsContext";
import { generateRoomCode, createRoom } from "../api/watchTogether";
import { findMovieOnArchive } from "../api/archive";
import {
    getMovieDetails,
    getMovieVideos,
    getMovieCredits,
...
export default function MovieDetail() {
    ...
    async function handleWatchTogether() {
        ...
        const userName = localStorage.getItem("cinehub_username")
            || prompt("What's your name?")
            || "Host";

        localStorage.setItem("cinehub_username", userName);

        const roomCode = generateRoomCode();
        const year = movie.release_date?.slice(0, 4);
        const archiveResult = await findMovieOnArchive(movie.title, year);

        await createRoom(
            roomCode,
            movie,
            videoKey,
            archiveResult?.identifier || null,
            userName
        );

        setCreatingRoom(false);
        navigate(`/watch/${roomCode}?name=${encodeURIComponent(userName)}`);
    }
                ...
                <div className="detail-main">
                    ...
                    <div className="detail-info">
                        ...
                        <div className="detail-actions">
                            ...
                            <button
                                className="watch-together-btn"
                                onClick={handleWatchTogether}
                                disabled={creatingRoom}
                            >
                                <FiUsers size={18} />
                                {creatingRoom ? "Creating Room..." : "Watch Together"}
                            </button>
                        </div>
